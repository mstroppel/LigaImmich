...
internal sealed class TagAssetsByEventTask : IScheduledTask
{
    ...
    public TagAssetsByEventTask(IImmichClient immichClient, ILogger<TagAssetsByEventTask> logger)
    {
        _immichClient = immichClient;
        _logger = logger;
    }

    public string Name => "TagAssetsByEvent";
    ...
    public async Task ExecuteAsync(CancellationToken cancellationToken)
    {
        ...
        {
            if (!EventTagResolver.TryResolveTag(folder, out var tagValue))
            {
                continue;
            }
